...
where $A_{\text{max}}$ is the maximum age. The prior is defined as a log-normal distribution with parameters $\mu = ln(5.4/A_{\text{max}})$ and $\sigma = 0.438$. Using a maximum age of 80 years, the point estimate and median of the prior for $M$ is `r mod_in$ctl$MG_parms['NatM_p_1_Fem_GP_1','INIT']` per year.
...
The fecundity-at-length was based on ovary samples collected by the SWFSC Santa Cruz lab in 2023 and 2024. The fecundity relationship for `r spp` was estimated to be equal to $`r mod_in$ctl$MG_parms['Eggs_alpha_Fem_GP_1', 'INIT']`L^`r mod_in$ctl$MG_parms['Eggs_beta_Fem_GP_1', 'INIT']`$ in millions of eggs where $L$ is fork length in cm (@fig-fecundity).  Of the females collected during the spawning months, 24 fish were at stage 2, with pre-fertilized vitellogenic eggs. 

### Growth (Length-at-Age)

The majority of length-at-age data available for California quillback rockfish were collected in hte alst five years. The California Cooperative Groundfish Survey is the only source of standardized  commercial sampling program is the only fishery-dependent

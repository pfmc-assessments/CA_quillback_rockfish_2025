## Biological Parameters {#sec-biology}
...
The majority of length-at-age data available for California quillback rockfish were collected in the last five years. The \gls{ccgs} is the only fishery-dependent survey that collects otoliths as part of the survey design and the majority of age data are combined into a growth fleet.  External estimates of growth from ages across the range of quillback rockfish do not indicate sexually dimorphic growth (Claire Stuart, Cal Poly Humboldt, pers. comm.). A number of the `r spp` collected for biological studies targeted females; therefore, combined with lack of evidence for sexual dimorphism, we did not estimate dimorphic growth for the California-specific data. Lengths from age-zero fish (N = 192) were available from a dissertation study [@baetscher_2019_larval] and were only used to inform initial parameters for growth that were estimated external to the base model. @baetscher_2019_larval trapped juvenile rockfish within Monterrey Bay using \gls{smurf} and divers with nets. All of the `r spp` were morphologically identified as gopher, copper or kelp rockfish, but were genetically identified as `r spp`.  

Age composition data used in the model were from break-and-burn otolith reads, as described in @sec-fish-dep. Aged `r spp` used in the assessment were aged by the \gls{cap} lab in Newport, Oregon. Within-lab ageing error was based on one primary age reader and a second reader producing double reads from 418 otoliths collected from California (@fig-age_error). 

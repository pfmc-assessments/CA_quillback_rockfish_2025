...
%%%%%% Cover image
$if(cover)$
\pdftooltip{\centering\includegraphics[width=5in]{$cover$}}{A photo of a quillback rockfish.}
...
%$it.name.literal$\textsuperscript{$for(it.affiliations)$$it.number$$sep$,$endfor$$if(it.attributes.equal-contributor)$\Yinyang$endif$}

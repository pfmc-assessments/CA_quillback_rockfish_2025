...
#| fig-alt: "Phase plot showing population trajectory above the spawning output target and below the overfishing target (i.e. lower right quadrant) in most years."
...
#| fig-alt: "Time series plot showing fishing intensity above the SPR target of 50 percent in many years from 1980 to 2000, and again from 2015 to 2020."
...
During the last 10 years when `r spp` in California was managed within the Minor Nearshore Rockfish complex, the total removals were above the species' \gls{ofl} contribution from 2015--2023. Since 2023, `r spp` in California have had species-specific management measures but did not have species-specific harvest specifications. Total removals were below the \gls{ofl} contribution in 2024 (@tbl-es-management). 
...
#| warning: false
#| echo: false
#| tbl-cap: "Recent trend in overfishing limit (OFL), acceptable biological catch (ABC), and annual catch limit (ACL; set to equal ABC) contributions for quillback rockfish, along with total removals (landings + dead discards) all in metric tons (mt). For 2015-2022, values for OFLs and ACLs represent OFL and ACL contributions of quillback rockfish within the Minor Nearshore Rockfish North and South complexes. The ACL contribution allocated to California across both complexes is also provided and marked with an asterisk ($\\ast$), and is calculated as described in Section 1.5. For 2023-2024, California quillback rockfish remained in the complexes but was managed with statewide values based on the 2021 rebuilding analysis, so OFL, ABC, and ACL contributions in these years are provided as California only values."
#| tbl-pos: H
...
The 2025 stock assessment for `r spp` off California was assigned a category 1b determination by the \gls{ssc} to the \gls{pfmc}. A ten-year projection of the \gls{ofl}, \gls{abc}, and \gls{acl}, all based on a $P^*$ of 0.45 and a log-space standard deviation of the overfishing limit sigma ($\sigma$) of 0.75 is included in @tbl-es-projections. Assumed catches for 2025 and 2026 for this projection were provided by the \gls{gmt}, and catches from 2027 onward assume full attainment of the \gls{abc}, assuming the \gls{acl} is equal to the \gls{abc}.
...
The model estimate of the log-scale standard deviation of the overfishing limit (OFL) in 2025 is `r round(mod_out$OFL_sigma, 3)`, lower than the endorsed SSC value of 0.75, so harvest projections assume an initial $\sigma$ of 0.75. The model estimate of uncertainty around the 2025 spawning output is `r round(mod_out$Pstar_sigma, 3)`. Each of these underestimates the overall uncertainty due to the necessity to fix several key population dynamics parameters (e.g., steepness, recruitment variance, natural mortality) and also because there is no explicit incorporation of model structural uncertainty (although see the decision table for alternative states of nature).

...
The base model fixed parameters and estimates along with approximate asymptotic standard errors are shown in @tbl-parms and the likelihood components are shown in @tbl-likelihood. Estimates of stock size and status over time are shown  @tbl-timeseries. 
...
The aggregated fits to the length composition are reasonable @fig-lencomp-agg given the overall sparcity of data and variable sample sizes by year.  

The fits to the ROV and CCFRP surveys did not change significantly with changes in selectivity patterns. The higher Pearson residuals for CCFRP in 2017, 2018 and 2024 around 30 cm are likley due to the fact that the Farallons were sampled in these years and have high densities of quillback rockfish.  The ROV survey lengths were fit to the invidiual years of data and indicate an increase in mean size form 2019 to 2021 that was not captured by the model. The CCFRP mean lenght was uncertain across years and the increased mean lenght in 2019 was not well fit.

 The mean lengths observed by year from the commercial fleet were uncertain but the time-varying selecitivty captures the changes in mean length relatively well in all but one year, 2007. The commercial lengths samples from 2010--2014 only represent the two most northern ports, but there were not regulatory changes that supported the observed patter in increasing length. There were limited aged available for the commercial fleet from a limited number of years. There are a few remaining large Pearson residuals in individual years, but the mean age from the conditional data were well captured by the model @fig-caal-fit-comm.

There were no ages available from the recreational fleet collect as part of the CDFW \{crfs} sampling program. The model fit the tails of the recreational length composition well, but expected a peak length larger than the data suggested.  For years in which the large sample sizes of recreational lengths the data were well fit. The Pearson residuals were variable by year and no patterns of general misfit are visible. The mean length in the recreational fishery was more uncertain prior to 1993.  The mean length in 1983 was not fit with marked higher Pearson residuals for lareger fish in the early 1980s.  There were no regulations nor major changes in the recreational fishery prior to 1999 that would suggest the need for an additional time block to aid in fitting these data. 
...
The relative indices from CCFRP and the ROV survey both indicated increases in the later years of the survey that the model fit underestimates. However, both indices were fit well without hte need to add extra variation to capture changes.  The decreased value in 2019 for CCFRP was not captured by the model and may be a result of factors not considered in index development, i.e., higher than average catches of a midwater species.

...
ca_catch$rec_tot[ca_catch$Year==1983] <- mean(ca_catch$rec_lan[ca_catch$Year %in% years])

# Adjust the 1993 value to be an average of the 3 years after
years <- c(1994, 1995, 1996)
ca_catch$rec_tot[ca_catch$Year==1993] <- mean(ca_catch$rec_tot[ca_catch$Year %in% years])
...
#---------------------------------------------------------------------------------------------------------------#
write.csv(ca_catch, "ca_catch_rec_sens.csv")

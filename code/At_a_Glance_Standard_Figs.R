##########################################################################################-
#
# Figure material for the At-A-Glance Summaries 
# 
# Originally used Aaron's sablefish scripts. 
# Edited to apply to 2025 California Quillback Rockfish 
#
...
#Output materials into this folder 
dir.create(here::here("presentations", "At_A_Glance"))


############################################
############################################
#1. Plot removals figure (front page top panel)
#Version in report done in model_runs.R (search for 'catch2_landings_stacked_custom.png')
model_output <- r4ss::SS_output(here::here("models", base_mod_name))

removals <- model_output$catch |> 
  dplyr::mutate(
    year = Yr,
    catch_mt = dead_bio,
    Fleet = dplyr::case_when(
      Fleet_Name == "CA_Commercial" ~ "Commercial",
      Fleet_Name == "CA_Recreational" ~ "Recreational",
      .default = Fleet_Name)
  )

ggplot2::ggplot(removals, ggplot2::aes(x = year, y = catch_mt, fill = Fleet)) +
  ggplot2::geom_bar(stat = 'identity') +
  ggplot2::theme_bw() +
  ggplot2::xlab("Year") + ggplot2::ylab("Total removals (mt)") +
  ggplot2::xlim(c(1916, 2025)) + 
  ggplot2::scale_y_continuous(
    labels = function(x) format(x,scientific = FALSE)) +
  #nmfspalette::scale_fill_nmfs(palette = "waves", reverse = TRUE) +
  #ggplot2::scale_fill_viridis_d() +
  ggplot2::scale_fill_manual(values = c(r4ss::rich.colors.short(2))) +
  ggplot2::theme(
...
############################################
############################################
#2. Plot historical comparisons figure (front page mid panel)
#Version in report done in model_runs.R (search for 'historical_comparison.png')
model_output <- r4ss::SS_output(here::here("models", base_mod_name))
old_model_output <- r4ss::SS_output(here::here("models", "0_0_2_2025setup"))
...
sumbio <- rbind(
  model_output$timeseries |> 
    ...
    dplyr::mutate(
      year = Yr,
      model = "2025 base model"
    ),
  old_model_output$timeseries |> 
    dplyr::select(Yr, Bio_smry) |>
    dplyr::mutate(
      year = Yr,
      model = "2021 model"
...
ggplot2::ggplot(sumbio, ggplot2::aes(x = year, y = Bio_smry, color = model)) +
  ggplot2::geom_line(lwd = 1.5) +
  ggplot2::theme_bw() +
  ggplot2::xlab("Year") + ggplot2::ylab("Age 3+ biomass (mt)") +
  ggplot2::xlim(c(1916, 2025)) + 
  ggplot2::scale_y_continuous(
    labels = function(x) format(x,scientific = FALSE)) +
  #nmfspalette::scale_fill_nmfs(palette = "waves", reverse = TRUE) +
  #ggplot2::scale_fill_viridis_d() +
  ggplot2::scale_color_manual(values = c(rev(r4ss::rich.colors.short(2)))) +
  ggplot2::theme(
    strip.text.x = ggplot2::element_blank(),
    strip.background = ggplot2::element_rect(colour="white", fill="white"),
    legend.key.height = ggplot2::unit(0.05, "cm"),
...
############################################
############################################
#3. Plot summary biomass status with error and states of nature (front page bottom panel)
#No version in report but version in GFSC presentation done in decision_table.R

model_output <- SS_output(here('models', base_mod_name))
model_output_high <- SS_output(here('models', paste0(base_mod_name, "_high_M_pstar_0.45")))
model_output_low <- SS_output(here('models', paste0(base_mod_name, "_low_M_pstar_0.45")))
...
p1 <- ggplot() +
  ...
  geom_polygon(data = data.frame(x = c(filter(bratio_hi,Yr>1994 & Model=="model1")$Yr, rev(filter(bratio_hi,Yr>1994 & Model=="model1")$Yr)),y = c(filter(bratio_hi,Yr>1994 & Model=="model1")$Bratio, rev(filter(bratio_lo,Yr>1994 & Model=="model1")$Bratio))),
...
p2 <- ggplot() +
  ...
  ggplot2::theme(
    ...
    plot.margin = unit(c(2,2,-15,-10), "points"),
...
#-----------------------------------------------------------#
#
#The following plots are ones for sablefish that we dont use
#
#-----------------------------------------------------------#
